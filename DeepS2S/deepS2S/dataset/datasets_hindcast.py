...
class WeatherDataset(pl.LightningDataModule):
    def __init__(self, dataset_name, data_info, var_comb, seasons=None, return_dates=False):
        ...
        self.out_step = data_info['config'].get('out_step', True)
        ...
        if 'Hindcast' in self.var_comb['input']:
            self.m_days = 1
        ...
        for var_name, info in data_info['vars'].items():
            ...
            if var_name == 'nae_regimes':
                ...
                path_time = f'~/WiOSTNN/Version1/data/{dataset_name}/datasets/{self.regime_path}z_{pressure_level}_{resolution}deg_{years}_{region}_2d_NAEregimes_lead0.nc'
                
            elif var_name == 'Hindcast':
                resolution = 2.5
                pressure_level = info.get('pressure_level', '')
                region = info['region']
                path = f'~/WiOSTNN/Version1/data/{var_name}/datasets/z_{pressure_level}_{resolution}deg_{years}_{region}_2d_NAEregimes.nc'
                print(path)
            ...
            if data_type == 'categorical':
                ...
                if 'index' in var_name and 'mjo' in region:
                    ...
                    ds_norm = ds.transpose('time', 'nae_regime_cat')
                else:
        ...
        # if 'Hindcast' in var_comb['input']:
        #     times = np.unique(self.inputs['1d'][0].time.values)
        #     for keys, data in self.inputs['2d'].items():
        #         for i, d in enumerate(data):
        #             self.inputs['2d'][keys][i] = self.inputs['2d'][keys][i].sel(time=times, method = "nearest")
        #     self.output = self.output.sel(time=times, method = "nearest")
    ...
    def __getitem__(self, idx):
        ...
        inputs = self.multiple_input(season, season_idx, idx, output_slice)
    ...
    def multiple_input(self, season, season_idx, idx, output_slice):
        ...
        inputs = self.input_1d(inputs, season, idx, output_slice)
    ...
    def input_1d(self, inputs, season, idx, output_slice):

        out_idxs = output_slice.time.values[-1]
            ...
            if 'MJO' in self.data_info[nd]:
                ...
                if nd == 0:
                    ...
                    inp = torch.tensor(d.sel(time=d.season == season).isel(time=in_idxs).values[:,None], 
                            dtype=torch.float32)
                    inps = torch.cat((inps, inp), dim = 1)
            elif 'Hindcast' in self.var_comb['input']:
                
                if nd == 0:
                    inps = torch.tensor(d.sel(time=out_idxs, method = "nearest").values,
                    dtype=torch.float32)
                else:
                    inp = torch.tensor(d.sel(time=out_idxs, method = "nearest").values,
                           dtype=torch.float32)
                    inps = torch.cat((inps, inp), dim = 1)
            else:
                if nd == 0:
                    inps = torch.tensor(d.sel(time=d.season == season).isel(time=out_idxs).values,
                            dtype=torch.float32)
                else:
                    inp = torch.tensor(d.sel(time=d.season == season).isel(time=out_idxs).values,
                           dtype=torch.float32)
                    inps = torch.cat((inps, inp), dim = 1)
